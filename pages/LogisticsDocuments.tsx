    ...
    return (
        <div className="space-y-6">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-3">
                        <div className="p-2 bg-gradient-to-r from-orange-500 to-amber-500 rounded-xl text-white">
                            {activeType === 'BL' ? <FileText size={24} /> : <ClipboardList size={24} />}
                        </div>
                        {activeType === 'BL' ? 'Bill of Ladings' : 'Booking Confirmations'}
                    </h1>
                    <p className="text-slate-500 text-sm mt-1">Manage extracted logistics documents</p>
                </div>
                <div className="flex items-center gap-3">
                    {((activeType === 'BOOKING' && onSaveBooking) || (activeType === 'BL' && onSaveBL)) && (
                        <button
                            onClick={() => setShowOcrModal(true)}
                            className="flex items-center gap-2 px-5 py-2.5 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition-all shadow-md font-medium"
                        >
                            <ScanEye size={18} />
                            OCR Upload
                        </button>
            ...
            <div className="flex gap-2 w-full md:w-auto mb-4">
                <div className="relative flex-1 md:w-64"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="text" placeholder="Search documents..." className="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-in zoom-in-95">
                        ...
                        <div className="p-6 space-y-4 overflow-y-auto custom-scrollbar">
                            <div className="grid grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    <select className="w-full border border-slate-300 rounded p-2 text-sm" value={bookingFormData.salesOrderId || ''} onChange={e => setBookingFormData(p => ({ ...p, salesOrderId: e.target.value }))}>
                                        ...
                                        {[...salesOrdersList].sort((a, b) => a.orderNumber.localeCompare(b.orderNumber)).map(so => <option key={so.id} value={so.id}>{so.orderNumber}</option>)}
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <select className="w-full border border-slate-300 rounded p-2 text-sm" value={bookingFormData.companyId || ''} onChange={e => setBookingFormData(p => ({ ...p, companyId: e.target.value }))}>
                                        ...
                                        {[...companiesList].sort((a, b) => a.name.localeCompare(b.name)).map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                                    </select>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select className="w-full border border-slate-300 rounded p-2 text-sm" value={bookingFormData.customer || ''} onChange={e => setBookingFormData(p => ({ ...p, customer: e.target.value }))}>
                                        ...
                                        {[...customersList].sort((a, b) => a.name.localeCompare(b.name)).map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                                    </select>
                                </div>
                                <div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Port of Loading</label><select className="w-full border border-slate-300 rounded p-2 text-sm" value={bookingFormData.pol} onChange={e => setBookingFormData(p => ({ ...p, pol: e.target.value }))}><option value="">Select Port</option>{[...ports].sort((a, b) => a.name.localeCompare(b.name)).map(port => <option key={port.id} value={port.code}>{port.name} ({port.code})</option>)}</select></div>
                                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Port of Discharge</label><select className="w-full border border-slate-300 rounded p-2 text-sm" value={bookingFormData.pod} onChange={e => setBookingFormData(p => ({ ...p, pod: e.target.value }))}><option value="">Select Port</option>{[...ports].sort((a, b) => a.name.localeCompare(b.name)).map(port => <option key={port.id} value={port.code}>{port.name} ({port.code})</option>)}</select></div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    >
                                        ...
                                        {[...ports].sort((a, b) => a.name.localeCompare(b.name)).map(port => <option key={port.id} value={port.code}>{port.name} ({port.code})</option>)}
                                    </select>
                                </div>
                                <div>
