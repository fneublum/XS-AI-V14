...
const App: React.FC = () => {
    ...
    const billOfLadings = useSupabase<BillOfLading>('bill_landings', {
        select: 'id, companyId, createdAt, blNumber, shipper, consignee, notifyParty, vesselVoyage, portLoading, portDischarge, placeReceipt, placeDelivery, shippedDate, eta, originals, container, seal, description, grossWeight, measurement, packages, freightPayable, remarks, status, originalDocument'
    });
    ...
    const invoices = useSupabase<Invoice>('invoices', {
        select: 'id, companyId, createdAt, invoiceNumber, invoiceDate, shipperName, shipperAddress, soldTo, shipTo, paymentTerms, incoterm, dateOrder, customerPo, carrier, transportRef, freightTerms, items, grossWeight, netWeight, tareWeight, totalQuantity, subtotal, totalAmount, currency, remitTo, bankName, bankAddress, swiftCode, routingNumber, accountNumber, supplier, date, soNumber, originalDocument, bookingNumber, shipper, consignee, pod, containers, plNumber, billToName, memo, bolUrl'
    });
    ...
    const commissions = useSupabase<CommissionSalesOrder>('commission_sales_orders', {
        select: 'id, orderNumber, invoiceNumber, sellerName, customerName, pod, deliveryDate, status, orderTotal, invoiceStatus, commissionAmount, commissionPaymentStatus, createdAt, companyId, items, totalQuantity',
        limit: 100,
