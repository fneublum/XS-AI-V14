...
import { CargoAgent, Company } from '../types';
import { Search, Plus, X, Save, Pencil, Trash2, Truck, Mail, Phone, Globe, LayoutGrid, List, User, Hash, Filter, CheckCircle2, FilePlus } from 'lucide-react';

interface CargoAgentsProps {
    agents: CargoAgent[];
    onAdd: (a: CargoAgent) => void;
    onUpdate: (a: CargoAgent) => void;
...
const CargoAgents: React.FC<CargoAgentsProps> = ({ agents, onAdd, onUpdate, onDelete, currentCompanyId, availableCompanies, initialViewMode = 'grid' }) => {
    ...
    const initialFormState: Partial<CargoAgent> = {
        name: '',
        code: '',
        contact: '',
        email: '',
    ...
    const [formData, setFormData] = useState<Partial<CargoAgent>>(initialFormState);
    ...
    const handleDelete = (id: string) => {
        if (window.confirm('Delete this cargo agent?')) {
            onDelete(id);
        }
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            onUpdate({ ...formData, id: editingId, companyId: targetCompanyId } as CargoAgent);
        } else {
            const newCode = `AGT-${Math.floor(1000 + Math.random() * 9000)}`;
    ...
    const renderColumnHeader = (id: keyof CargoAgent, label: string) => {
        const uniqueValues = getUniqueValues(agents, id as string);
            ...
            {viewMode === 'grid' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filteredAgents.map(agent => (
                        <div key={agent.id} className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 hover:shadow-md transition-shadow group relative">
                            <div className="flex justify-between items-start mb-2">
                                <div>
                                    <h3 className="font-bold text-slate-800 text-lg">{agent.name}</h3>
                                    {agent.code && <span className="text-xs font-mono text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded">{agent.code}</span>}
                                </div>
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => handleEdit(agent)} className="text-slate-400 hover:text-amber-500"><Pencil size={16} /></button>
                                    <button onClick={() => handleDelete(agent.id)} className="text-slate-400 hover:text-red-500"><Trash2 size={16} /></button>
                                </div>
                            </div>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filteredAgents.map(agent => (
                                <tr key={agent.id} className="hover:bg-slate-50 transition-colors group">
                                    <td className="px-3 py-2 text-sm font-mono text-slate-500">{agent.code || '-'}</td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-white rounded-xl shadow-2xl max-w-md w-full overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center p-4 border-b border-slate-100">
                            <h3 className="font-bold text-slate-800">{editingId ? 'Edit Agent' : 'Add Cargo Agent'}</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <div>
                                ...
                                <input required className="w-full border border-slate-300 rounded-lg p-2 text-sm" name="name" value={formData.name} onChange={handleInputChange} placeholder="e.g. DHL Global" />
                            </div>
