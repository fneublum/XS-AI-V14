...
const getOutlookToken = async (tenantId: string, clientId: string, clientSecret: string): Promise<string> => {
    // In production, skip verification to avoid CORS proxy issues
    if (isProduction) {
        console.log('[AiEmailAssistant] Production detected - using stored credentials without verification');
        // Return a placeholder - actual authentication will use MSAL when needed
        throw new Error('PRODUCTION_SKIP');
    }
    ...
    const scanEmails = async (boxOverride?: 'INBOX' | 'DELETED') => {
        ...
        for (const acc of accounts) {
            if (acc.provider === 'OUTLOOK') {
                try {
                    // Get tenantId and clientId from direct properties (new format) or refreshToken (legacy)
                    let tenantId = acc.tenantId;
                    let clientId = acc.clientId;
                    let clientSecret = acc.clientSecret || acc.password;

                    // Fallback to parsing refreshToken for legacy accounts
                    ...
                    if (!tenantId || !clientId) {
                        console.error(`Missing tenantId or clientId for ${acc.email}`);
                        continue;
                    }

                    let token = '';
                    try {
                        token = await getOutlookToken(tenantId, clientId, clientSecret);
                    } catch (err: any) {
                        if (err.message === 'PRODUCTION_SKIP') {
                            // In production, skip token verification and rely on Graph API with interactive auth
                            console.log(`[AiEmailAssistant] Skipping token verification for ${acc.email} in production`);
                            // Continue without token - we'll need to handle this differently in production
                            continue;
                        }
                        throw err;
                    }
                ...
                if (acc.provider === 'OUTLOOK') {
                    ...
                    if (tenantId && clientId) {
                        try {
                            ...
                            if (err.message === 'PRODUCTION_SKIP') {
                                console.log(`[AiEmailAssistant] Production mode - skipping token verification for ${email.accountEmail}`);
                                // In production, we can't verify tokens via CORS proxy
                                // The email service will handle authentication via MSAL
                                throw new Error('Cannot process emails in production without proper MSAL setup');
                            }
                            throw err;
                        }
                    }
