        ...
        <div className="relative min-h-[calc(100vh-8rem)] flex flex-col gap-6">
            <div className="mb-6 flex items-start justify-between">
                <div className="flex items-center gap-3">
                    <div className="p-2 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-xl text-white"><Calculator size={24} /></div>
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800">Quote Engine</h1>
                        <p className="text-slate-500 text-sm mt-1">Build and manage customer quotes</p>
                    </div>
                </div>
            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                >
                                    ...
                                    {[...customers].sort((a, b) => a.name.localeCompare(b.name)).map(c => <option key={c.id} value={c.id}>{c.name} {c.nickname ? `(${c.nickname})` : ''}</option>)}
                                </select>
                            </div>
                            <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                            <div className="md:col-span-4">
                                ...
                                >
                                    ...
                                    {[...products].sort((a, b) => a.name.localeCompare(b.name)).map(p => <option key={p.id} value={p.id}>{p.name} - {p.grade}</option>)}
                                </select>
                            </div>
