                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div>
                            ...
                            <select required name="customerId" value={formData.customerId || ''} onChange={handleInputChange} className="w-full pl-9 pr-3 py-2 border border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm appearance-none bg-slate-900 text-white">
                                ...
                                {customers.filter(c => currentCompanyId === 'ALL' ? (formData.companyId ? c.companyId === formData.companyId : true) : true).map(c => <option key={c.id} value={c.id}>{c.name} {c.nickname ? `(${c.nickname})` : ''}</option>)}
                            </select>
                            </div>
                            ...
                            <div className="grid grid-cols-1 gap-2 mb-3">
                                <div>
                                    ...
                                    >
                                        ...
                                        {products.map(p => <option key={p.id} value={`${p.name} - ${p.grade}`}>{p.name} ({p.grade})</option>)}
                                    </select>
                                </div>
