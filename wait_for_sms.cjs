...
async function main() {
    try {
        ...
        console.log("Listening for new SMS verification codes on +19302007070 (polling for 30s)...");

        // Loop for ~30 seconds, checking every 3 seconds
        let checks = 0;
        ...
        while (checks < 10) {
            ...
            for (const m of (j.messages || [])) {
                if (m.direction === 'inbound') {
                    if (checks === 0) {
                        // First run, populate known messages
                        knownSids.add(m.sid);
                        ...
                        // We found a code! Stop listening
                        process.exit(0);
                    }
                }
            }
        ...
        console.log("\nNo new SMS received in the last 30 seconds.");
