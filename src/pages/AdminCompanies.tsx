...
import { Building, Plus, X, Save, Trash2, Pencil, MapPin, Tag } from 'lucide-react';
...
const AdminCompanies: React.FC<AdminCompaniesProps> = ({ companies, onAdd, onUpdate, onDelete }) => {
  ...
  const [formData, setFormData] = useState<Partial<Company>>({ 
      name: '', nickname: '', address: '', city: '', state: '', zip: '', country: ''
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setIsAdding(false);
    setEditingId(null);
    setFormData({ name: '', nickname: '', address: '', city: '', state: '', zip: '', country: '' });
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
           ...
           <p className="text-slate-500 text-sm">Create and manage tenant organizations</p>
        </div>
        <button onClick={handleAddNew} className="flex items-center gap-2 bg-slate-800 text-white px-4 py-2 rounded-lg hover:bg-slate-900 transition-colors">
