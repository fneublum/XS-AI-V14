...
const LogisticsDocuments: React.FC<LogisticsDocumentsProps> = ({
    ...
    // Ensure default sort is applied when switching to Bookings
    useEffect(() => {
        // Debug logging
        // console.log('LogisticsDocuments Effect - activeType:', activeType, 'Current Sort:', sortConfig);

        if (activeType === 'BOOKING') {
            // Apply default sort if none exists OR if we switched types and want to enforce it
            // Using functional update to avoid dependency cycle if we wanted to check prev state detailedly
            setSortConfig(prev => {
                // If already sorted by ETD ASC, leave it alone (allows user to toggle desc without fighting effect?)
                // NO, "Always sort default" means on SWITCH/MOUNT.
                // If user changes it, we don't want to revert it immediately unless activeType changed.
                // But this effect runs on [activeType]. 
                // So if activeType stays 'BOOKING', it won't re-run.
                // If activeType just changed to 'BOOKING', force default.
                return { key: 'etd', direction: 'asc' };
            });
        } else {
            setSortConfig(null);
        }
