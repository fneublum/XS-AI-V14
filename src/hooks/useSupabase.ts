  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (tableName === 'bookings') {
        console.log(`[useSupabase FETCH] ${tableName}: rows=${dbData?.length ?? 'null'}, error=${dbError?.message ?? 'none'}`);
        if (dbData && dbData.length > 0) {
          console.log(`[useSupabase FETCH] ${tableName} sample:`, (dbData as any[]).slice(0, 2).map((r: any) => ({ id: r.id, companyId: r.companyId })));
        }
      }
