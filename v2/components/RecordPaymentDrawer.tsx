...
const buildReceiptOcrSpec = (mode: 'receipt' | 'payment'): ExtractSpec<ReceiptOcrFields> => ({
  prompt: [
    ...
    `Given a payment confirmation document — bank wire confirmation, ACH`,
    `receipt, deposit slip, vendor receipt, OR a Brazilian FX exchange`,
    `contract ("Contrato de Câmbio" / "Aviso de Operação de Câmbio") which`,
    `EC4's clients send when paying in BRL via a Brazilian bank — extract`,
    `the following fields into a single JSON object. Return ONLY the JSON,`,
    `no other text.`,
    ``,
    `Schema:`,
    `{`,
    ...
    `  "reference": string|null,      // see "Reference field" below`,
    `  "invoiceNumberHint": string|null,`,
    `  "memo": string|null`,
    `}`,
    ``,
    ...
    `  Brazilian FX contracts (Contrato de Câmbio, câmbio pronto) → WIRE.`,
    `- Strip currency symbols and thousand separators from amount.`,
    `  Brazilian receipts use "." as thousands and "," as decimal`,
    `  ("1.399.775,00" → 1399775.00). Detect and convert.`,
    `- Prefer ISO dates ("Jan 15, 2026" → "2026-01-15").`,
    `  Brazilian DD/MM/YYYY ("21/05/2026" → "2026-05-21"); never read`,
    `  "21/05/2026" as May 21st in US format — it IS May 21st but only`,
    `  because the Brazilian source uses DD/MM. "Data Operação" /`,
    `  "Data da Operação" / "Data Contratação" all map to txnDate.`,
    ``,
    `Reference field — capture the ONE strongest identifier:`,
    `- Bank wire: SWIFT / IMAD / OMAD / wire reference / transaction id.`,
    `- ACH: trace number / confirmation id.`,
    `- Check: check number.`,
    `- Brazilian FX contract: the CONTRACT NUMBER, labelled any of`,
    `  "Nro. do Contrato", "Número do Contrato", "Nº Contrato",`,
    `  "Contrato Nº", "No. Contrato", "Nr. Contrato" — the digits next`,
    `  to that label (e.g. "1399775") are the reference. ALWAYS prefer`,
    `  the contract number over operation number / boleto / DOC / TED`,
    `  numbers when both are present on a Contrato de Câmbio.`,
    ``,
    `Currency hint: Brazilian FX contracts typically settle USD or EUR`,
    `against BRL. The CONTRACT's currency is the foreign currency (USD),`,
    `not BRL — that's what counterparty actually paid us.`,
  ].join('\n'),
  normalize: (parsed) => {
